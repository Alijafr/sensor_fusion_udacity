...
public:
    ...
    void cluster_Helper(int index,const std::vector<std::vector<float>>& points,std::vector<bool> &processed, std::vector<int> &cluster ,KdTree* tree,float distanceTol);

    std::vector<std::vector<int>> euclidean_Cluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol);


    std::vector<typename pcl::PointCloud<PointT>::Ptr> Clustering_from_scratch(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize);
